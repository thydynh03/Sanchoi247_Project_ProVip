...
import com.example.SanChoi247.model.repo.RatingRepo;
...
public class BookingController {
    ...
    @PostMapping("/rate")
    public String rateBooking(@RequestParam("bookingId") int bookingId,
        ...
        rating.setStar(star);
        rating.setUid(uid);
        rating.setBooking_id(bookingId);
        ratingRepo.save(rating);

        model.addAttribute("message", "Your rating has been submitted successfully!");
        return "redirect:/";
    }
    ...
    public String showIndex(Model model) {
        ...
        model.addAttribute("averageRating", avgRating != null ? avgRating : "Chưa có đánh giá");
        System.out.println("Average Rating sent to index: " + avgRating); // Thêm dòng này để kiểm tra
        return "index"; // tên view (index.html)
    }
}