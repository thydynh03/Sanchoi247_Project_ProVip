...
@GetMapping("/blog")
    ...
    public String deletePost(@RequestParam("postId") Long postId, Model model, HttpSession session) {
        ...
        // Kiểm tra nếu người dùng không đăng nhập hoặc không có quyền truy cập
        if (currentUser == null) {
        ...
        // Chỉ cho phép xóa nếu là admin (role 'A') hoặc tác giả của bài viết
        if (currentUser.getRole() != 'A' && post.getAuthorId() != currentUser.getUid()) {
            return "redirect:/error"; // Chuyển hướng đến trang lỗi nếu không có quyền
        }
