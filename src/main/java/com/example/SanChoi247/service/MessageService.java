...
public class MessageService {
    ...
    // Lấy danh sách tin nhắn theo senderUid mà không cần JOIN
    public List<Message> getMessagesBySenderUid(int senderUid) {
        String sql = "SELECT * FROM messages WHERE sender_uid = ? ORDER BY timestamp ASC";
        return jdbcTemplate.query(sql, messageRowMapper, senderUid);
    }

    // Lấy danh sách tin nhắn theo receiverUid mà không cần JOIN
    public List<Message> getMessagesByReceiverUid(int receiverUid) {
        String sql = "SELECT * FROM messages WHERE receiver_uid = ? ORDER BY timestamp ASC";
